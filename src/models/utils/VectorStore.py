...
from models.utils.ModelVersion import BGE_SMALL, FAISS_PATH, LUKEC_FAISS_PATH
...
class LukecVectorStore:
    def __init__(self):
        self.store = LocalFileStore("./lukec_cache/")
        self.embedding_model_version = BGE_SMALL
    ...
    def create_vector_store(self, split_docs, verbose):
        if verbose:
            print("[VectorStore] Preparing FAISS Vector Store...")
        if os.path.exists(LUKEC_FAISS_PATH):
            self.vector_store = FAISS.load_local(LUKEC_FAISS_PATH, self.embedder, allow_dangerous_deserialization=True)
        else:
            self.vector_store = FAISS.from_documents(split_docs, self.embedder)
            self.vector_store.save_local(LUKEC_FAISS_PATH)
        if verbose:
            print("[VectorStore] Finished preparing FAISS Vector Store.")
